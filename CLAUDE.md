...
- **ステータス**: **審査中**（2026/06 の低品質コンテンツ対策後に再申請済み → 結果待ち）
  - 審査結果メールの宛先アカウント: `saboten829@gmail.com`（`adsense-noreply@google.com` から届く）
  - 過去の却下履歴: 2026/04/19・05/06・**05/16**（計3回却下）。いずれも 2026/06 の対策キャンペーン**前**。
  - ⚠️ 却下メール本文は定型文で**具体的理由は書かれない**。理由は AdSense 管理画面の **[サイト] ページ**に表示される。
- **設置範囲**: 全23ページの `<head>` にスクリプト設置済み
- **`ads.txt`**: プロジェクトルートに設置済み（2026/07）。内容は1行 `google.com, pub-9197151217236924, DIRECT, f08c47fec0942fa0`。
  - ※ `ads.txt` は**審査の合否には影響しない**（承認後の広告配信・収益化に必要）。承認後の手戻り防止のため先行設置。
...
## 2026/07 セッションで得た知見 — AdSense 審査状況の確認と ads.txt 設置

### AdSense 審査状況（メールボックスから確認）

Gmail（`saboten829@gmail.com`）を確認したところ、`adsense-noreply@google.com` から
**計4通**のメールを受信：

| 日付 | 内容 |
|---|---|
| 2026/04/12 | 申請受付（審査に送られました）|
| 2026/04/19 | ❌ 却下① |
| 2026/05/06 | ❌ 却下② |
| 2026/05/16 | ❌ 却下③（メール上の最新）|

**2026/05/16 以降、新しい AdSense 通知メールは届いていない。**
これら3回の却下はいずれも 2026/06 の低品質コンテンツ対策キャンペーン**より前**。
ユーザ確認済みで**6月改善後に再申請済み**のため、現状は**審査中（結果待ち）**と判断。

**重要**: 却下メールの本文は定型文で、具体的な却下理由は書かれていない。
理由は AdSense 管理画面の **[サイト] ページ**でしか確認できない。次に却下された場合は
まずこのページの文言を確認すること。

### ads.txt を設置（プロジェクトルート）

承認後の広告配信・収益化に必要な `ads.txt` を先行設置：
```
google.com, pub-9197151217236924, DIRECT, f08c47fec0942fa0
```
- ⚠️ `ads.txt` は**審査の合否には影響しない**（審査中は広告未配信のため無関係）。
  承認された瞬間から必要になり、無いと「収益に重大な影響」警告が出る。手戻り防止のため先行設置した。
- 反映確認: https://28tools.com/ads.txt で1行が表示されればOK。承認後の認識には最大24〜48h。

### 審査結果の「通知」について（運用メモ）

Claude セッションは常駐監視できない（コンテナは無操作で破棄される）ため、
**AI 側から能動的に審査結果を通知する仕組みはない**。運用は：
- 審査結果は Google から**ユーザ本人のメール**（`saboten829@gmail.com`）に直接届く。
- 気になったタイミングでユーザが声をかければ、その場で Gmail MCP で確認・要約する。

---

## 2026/07 セッションで得た知見 — 問い合わせ対応窓口メール `info@28tools.com` 新設 & AdSense状況の実確認

### 問い合わせフォーム経由の対応フロー（実例）
- フォーム（Formspree）にアドインの**ダウンロードパスワード**を尋ねる問い合わせが届いた際の対応記録。
- **アドインのDLパスワードは `28tools`**（`js/main.js` の `downloadConfig.password` と共通。限定公開ツールのロック解除も同じパスワード）。
- 案内した手順: `addins.html` を開く → Revitバージョンのタブ選択 → ダウンロードボタン → パスワード `28tools` 入力 → 対応ZIP（例 `28Tools_Revit2024_v2.0.zip`）DL開始。
- **返信文の方針（重要）**: 宛名は**問い合わせに書かれた氏名のみ**。会社名・業種など**こちらで調べた情報は返信に載せない**（相手が明かしていない情報を書かない）。あわせて「サイトを知ったきっかけ」「不具合・改善要望があれば返信を」の一文を添えると次につながる。
- Formspree通知はReply-Toが送信者に設定されるので、通知メールに返信すれば送信者に届く。
- **メール本文にURLを書くときは全角カッコ `（ ）` で囲まない**。メールソフトの自動リンクが `）` や後続文字まで巻き込んでクリック範囲が壊れる。**URLは独立行**に置く。

### サイト運営用メール `info@28tools.com` を新設（iCloud+ カスタムメールドメイン）
**背景**: 個人アドレスでの返信を避け、屋号アドレスで問い合わせ対応するため新設。
- ❌ **Zoho Mail 無料プランは日本（JAPANデータセンター）で新規受付終了**（[公式告知](https://support.zoho.co.jp/portal/ja/kb/articles/eos-of-zoho-mail-free-plan-for-new-signup)）。「Zohoで無料の独自ドメインメール」という旧情報は**今は不可**。料金ページにも無料枠は出ない。
- ✅ 採用: **iCloud+ Custom Email Domain**（月¥130〜のiCloud+加入者ならApple純正で送受信）。Mac/iPhoneの「メール」アプリで完結。

**設定方法（ネームサーバー移設は不要 = サイトへのリスクなし）**:
- ドメインはお名前.com管理（NS: `01〜04.dnsv.jp`）。**お名前.comの「DNSレコード設定」にメール用レコードを追加するだけ**（GitHub Pagesはそのまま）。
- iCloud側: `icloud.com/icloudplus` →「カスタムメールドメイン」で `28tools.com` を追加 →「メールアドレスの追加」で `info@28tools.com` を作成。
- **お名前.comに追加したレコード**:

| 種別 | ホスト名 | VALUE | 優先 |
|---|---|---|---|
| MX | 空欄（＝@） | `mx01.mail.icloud.com` | 10 |
| MX | 空欄（＝@） | `mx02.mail.icloud.com` | 10 |
| TXT | 空欄（＝@） | `apple-domain=…`（所有確認・アカウント固有） | — |
| TXT | 空欄（＝@） | `v=spf1 include:icloud.com ~all`（SPF） | — |
| CNAME | `sig1._domainkey` | `sig1.dkim.28tools.com.at.icloudmailadmin.com`（DKIM） | — |

- **お名前.com入力の落とし穴**: ①MX/CNAMEの**末尾「.」は付けない** ②SPFの前後の**「"」は付けない** ③ホストが「@」のものは**ホスト名欄を空欄** ④**SPF（`v=spf1…`）は1本だけ**（複数あると迷惑メール判定） ⑤**GitHub Pages用 A（`185.199.108〜111.153`）/ CNAME は消さない**（メール用と役割が別で共存可）。
- **反映確認**: `whatsmydns.net`（種別MX）で `mx01/mx02.mail.icloud.com` が各地点で緑になるか。Appleの「設定を完了」が **"MXレコードを確認してください"** で弾かれるのは**反映途中なだけ**。時間をおいて再度「確認」で通る（最大72h、実際は数十分〜数時間）。
- **送信**: Mac/iPhoneの「メール」で新規作成時、差出人（From）に `info@28tools.com` を選択。BCCに別プロバイダ（Gmail等）を入れて着信・迷惑判定を自己確認するとよい。

### AdSense 状況の実確認（従来の認識を訂正）
- AdSense管理画面 **[サイト]ページで 28tools.com が「ポリシー違反：有用性の低いコンテンツ（Low value content）」** と表示されていることを実確認。
- **以前の「審査中（結果待ち）」という認識は誤りで、実際は"修正待ち"**（違反解消 → 再審査リクエストが必要な状態）。
- 画面下部の「問題を修正しました」→「**審査をリクエスト**」は**まだ押していない**。
- 対応方針は既存の残タスク通り: **`news.html` / `ai-news.html` の「読み込み中…」を静的書き出しで解消**（マニュアル焼き込みと同じ思想）→ 本番反映を確認 → その後に「審査をリクエスト」を押す。

---

## 2026/07 セッションで得た知見 — X（旧Twitter）告知作戦の立案

### 背景・狙い
AdSense が「有用性の低いコンテンツ」で却下続き。真因は**インデックスの伸び悩み**
（登録済み2 / 未登録21）。オリジナル記事＋時間経過に加え、**外部シグナル（実アクセス）**を
作るため X で告知する作戦を立案した。**Claude は X 投稿できない**（環境に X 連携なし）ため、
成果物は「コピペで即投稿できる文面＋運用手順」。

### 成果物の場所
- **リポジトリ**: `marketing/x-promotion-plan.md`（ブランチ `claude/28tools-x-promotion-1kpx23` にコミット済み）。
  内容: アカウント設計（表示名 `28Tools｜Revit作図サポート`／bio／固定ツイート3案／アイコン方針）、
  **即投稿ツイート31本**（各≤140字・URL・ハッシュタグ込み）、初回2週間の投稿カレンダー、エンゲージ戦略。
- ツイートNo. 対応: 1-3=ツール横断／4-6=ハッチ／7-9=PDF比較／10-12=PDF→Excel／
  **12b-12d=AI議事録**／13-24=アドイン個別13機能／25-28=純Tips小ネタ。

### 重要な確認事項
- ⚠️ **AI議事録（`/ai-minutes/web/`）はパスワード保護を解除済み・一般公開中**とソースで確認
  （サイトアクセス用ゲートは存在せず、`password` 参照は Gemini APIキー入力欄のみ。sitemap・sidebar・
  index nav に公開リンクあり）。**CLAUDE.md 冒頭の「限定公開（パスワード保護）」記述は古い**ので、
  一般公開前提で扱う（X告知対象に含めた）。
- 告知対象URLは sitemap.xml と一致。梁色分けは統合後の `manual/beam-level-color.html` を使用。

### 未完了タスク（次セッションで実施）
- **Notion への転記が未完了**。「28tools 開発ハブ」ページ（Notion page_id `370d9148-e297-8186-9c63-e60ae9c79b01`）
  配下に、`marketing/x-promotion-plan.md` の全文で新規サブページ
  「X（旧Twitter）告知作戦（28tools.com / 2026-07）」を作成する。
- 今セッションでは Notion 書き込み（`notion-create-pages`）の**承認カードが描画されず**書き込めなかった
  （`requires approval` / `permission stream closed`。読み取り `notion-search` は成功）。ペイロード
  サイズ・内容は無関係と確認済み＝**承認UIそのものの一時不具合**。
- 対策として **`~/.claude/settings.json` を新規作成し、Notion書き込みを事前許可済み**:
  `permissions.allow` に `mcp__*__notion-create-pages` / `mcp__*__notion-update-page` /
  `mcp__*__notion-create-comment` を追加。**次セッションからは許可プロンプト無しで通るはず**。
  （このユーザー設定はリポジトリ外＝コミットされないので、環境が変わると消える点に注意。）

### X アカウント開設 & 流入計測の実装（2026/07 続き）

**Xアカウントを実開設**（ユーザーがブラウザ/アプリで作業、Claudeは伴走）:
- **@28tools_com**（表示名 `28Tools｜Revit作図サポート`）。場所=日本、Web=`https://28tools.com`。
- **bio**（採用＝数字なし・更新耐性版）: 「Revitの作図をラクにする無料ツールを作ってます🛠️｜アドイン＆ブラウザツールで、ハッチ作成・図面PDF比較・構造図のExcel化・AI議事録まで。インストール不要ですぐ試せます。作図時短のTipsも発信中(JP/EN/ZH)。要望はリプ/DMで👇」
- **アイコン**: サイトの favicon（`images/site-icons/favicon-28.png`＝青「28」/RGBA透過）を使用。⚠️ **透過PNGはXダークテーマで背景が黒く抜ける**ため、**白背景を焼き込んだ版**を作って設定（scratchpad で生成）。
- **固定ツイート**: 数字を出さない版（「Revitアドイン（対応バージョン幅広く）/ 自動ハッチング(.pat)作成 / 新旧PDF比較 / 構造図PDF→Excel / AI議事録」＋`28tools.com` ＋`#Revit #BIM`）。UTM無しだが **t.co リファラーで計測されるので貼り直し不要**（そのまま運用）。
- **ヘッダー画像は未設定**（青焼き方眼ベースで複数案を出したが、コピー/タイポが刺さらず保留。方眼の意匠は活かす方向）。
- 開設フロー中に **名前入力で「入力した名前を検証できません」が頻発** → これは**文字ではなくXの新規登録フロー側の一時不具合**（プレーン `28Tools` でも出る）。回避策＝一旦シンプル名で登録通過→後からプロフィール編集で正式名に変更。

**アドイン用語メモ**（bio/固定で使った表現）: 「Web工具」はダサいので不可 → **「ブラウザツール」**を使う。機能数（13等）や対応バージョン数字は**更新が面倒なので原則出さない**（「対応バージョン幅広く」等でぼかす）。

**X流入計測を実装済み**（本ブランチにコミット済み・**PR未マージ**）:
| 対応 | 内容 |
|---|---|
| `analytics/dashboard.html` | PVタブに **「X経由の訪問」KPIカード**（`id=kpi-pv-x`）を追加。Supabase `referrer`（t.co/x.com）＋ GA4 `source`（x/twitter/utm_source=x）を合算。判定は新設 `isXSource()` に集約 |
| `marketing/x-promotion-plan.md` | 全ツイート本文URL（32本）に `?utm_source=x&utm_medium=social` を付与（参照表は素のまま）。GA4での「X経由」分類とアプリ流入の取りこぼし対策 |

- ⚠️ `js/main.js` の `logPageview()` は `page` に**クエリを含めない**実装 → **utm自体はSupabaseに残らない**（Xは t.co リファラーで拾えるのでWeb流入はKPIに反映される）。アプリ流入までリアルタイムで完全計測したい場合は `logPageview` に `location.search` を送る小改修が必要（未実施）。
- **反映タイミング**: t.co経由のWeb流入はリアルタイム（Supabase）、GA4分は日次同期（`ga4-sync` cron）後に加算。

**残タスク**: ①このブランチのPRをマージして本番反映（dashboardのX KPI稼働）②X告知作戦をNotionへ転記（前掲）③ヘッダー画像の再検討。
---

## 2026/07 セッションで得た知見 — Notion 書き込みが失敗する根本原因と恒久対策

### 症状
Claude Code on the web（remote_mobile セッション）で **Notion への書き込み系 MCP ツール**
（`notion-create-pages` / `notion-update-page` 等）が失敗する。読み取り系
（`notion-fetch` / `notion-search`）は成功する。エラーは状況で2種類に変化する:
...
### 根本原因（実機調査で特定）
承認は**2層**あり、remote_mobile では両方が引っかかる。

1. **Claude Code 側の許可（permissions.allow）** — 書き込み系ツールは承認カードが必要。
   このセッションの `~/.claude/launcher-settings.json` は `permissions.allow: ["Skill"]` のみで、
   Notion 書き込みは未許可 → 承認カードを出そうとする。
2. **承認カードを表示・応答する websocket チャンネル（FD 3）が不安定**。
   MCP サーバの断続的な切断（Notion/GitHub/Supabase が数十秒おきに再接続）と連動して
   この承認チャンネルも落ちる。落ちている間は**承認が必要なあらゆる操作**が
   `permission stream closed` で失敗する（Notion 以外・組み込み Write ツールでも再現）。
3. layer1 を `permissions.allow` で解消すると、今度は Notion コネクタ（claude.ai 側 shttp）が
   **サーバ側で** `MCP tool call requires approval` を返す（コネクタ独自の同意ゲート）。

**決定的な切り分け**: `permissions.allow` に Notion ツールを足すと、エラーが
`permission stream closed`（layer1 のカード崩壊）→ `requires approval`（layer2 のコネクタ同意）に
**変化する**。これで layer1 は解消できたが layer2 が残る、と確定できる。

### 重要な事実
- ⚠️ **リモートのコンテナは毎回まっさら**。前セッションが `~/.claude/settings.json` に書いた
  事前許可は**次のコンテナで消える**。ユーザ設定での恒久化は不可能。
  → **恒久化はリポジトリにコミットする `.claude/settings.json`（プロジェクト設定）だけ**。
- ⚠️ **Notion コネクタのサーバ名（MCP server id）はセッション/再接続ごとに変わる**。
  セッション開始時はフレンドリ名 `mcp__Notion__*`、再接続後は UUID `mcp__<uuid>__*`。
  そのため許可ルールは**サーバ単位のフレンドリ名 `mcp__Notion`** を使う（UUID 固定は無意味）。
  セッション開始直後の最初の呼び出しはフレンドリ名なので `mcp__Notion` で一致する。
- ⚠️ `permissions.defaultMode: "bypassPermissions"` は**効かない**（managed policy で無効化済みらしく、
  かえって `permission stream closed` に逆戻りする）。使わないこと。
...
### 残る layer2 の解消方法（ユーザの1回だけの操作）
Notion コネクタのサーバ側同意は **claude.ai の設定に紐づく**ため、承認カードが正常に出る
**PC のブラウザ（claude.ai/code）または Claude デスクトップアプリ**で1回だけ承認すればよい:
1. claude.ai を**PC ブラウザ/デスクトップアプリ**で開く（モバイル/リモートだとカードが出ない）。
2. Notion 書き込みを1回実行 → 出た承認カードで **「常に許可（Always allow）」** を選ぶ。
