...
## 2026/07 セッションで得た知見 — AdSense 4回目却下対応 & Tips（使い方ガイド）新設

作業ブランチ: `claude/adsense-earnings-potential-8wsla2`

### AdSense 審査状況（2026/07/04 時点）
- **4回目の却下**。ステータス「要確認」／詳細 **「有用性の低いコンテンツ（low value content）」**。`ads.txt` は承認済み（審査に無関係）。
- **真因の特定（Search Console）**: `ページのインデックス登録` レポートで **登録済み2 / 未登録21**。
  未登録の理由は **「検出 - インデックス未登録」** と **「クロール済み - インデックス未登録」**（ソース: Google システム）。
  = **技術エラーではなく、Google自身がサイトを“価値/優先度が低い”と判断**している状態。AdSenseの低品質判定と同根。
- **結論**: ボタン一発では直らない。**時間＋オリジナルコンテンツ増＋外部シグナル（アクセス/被リンク）**で
  「クロール済み-未登録」→「登録済み」へ増やす必要。**再審査はインデックス済みが数ページに増えてから**（数週間単位）。

### GA4 実データ（AdSense収益試算の根拠）
- 本番解析は Supabase プロジェクト `clyfrkwuajbauqxvrfyf`（28yu's Project）。
- **アクセス手段**: 既存の Supabase MCP は別アカウント(dodeca-dev)で本番不可。→ **カスタムコネクタ**で
  `https://mcp.supabase.com/mcp?project_ref=clyfrkwuajbauqxvrfyf&read_only=true` を追加（名前 `Supabase-28tools`）。
  28yuアカウントでOAuth認証すれば `execute_sql` で本番を直接集計できる（dodeca-devと併存可）。
- **月次PV（GA4 `ga4_history`）**: 2026-06=**563 PV**、3ヶ月平均≈381。日本74%。検索流入は約15%と弱い。
- **月10万円試算**: RPM ¥300〜1000 で **月20〜33万PV必要（現状の約180〜590倍）**。**AdSense単独では非現実的**。
  現実的マイルストーン: 1,000→3,000→10,000 PV。10万円は収益複線化（アドイン有償版/アフィリエイト等）が前提。

### Tips（使い方ガイド）新設 — `tips/`
- **スコープ規則（重要）**: **Tips は「Webブラウザで使えるツール」限定**（ハッチング / PDF比較 / Excel抽出）。
  **アドイン機能の使い方は `manual/*.html`（アドインページの機能一覧からリンク）が担う**。混同しない。
  → 一度作った add-in 記事（grid-bubble / sheet-creation）は manual と重複するため**削除済み**。
- 現行 Tips（3本、いずれも静的HTML＋Breadcrumb/HowTo/FAQ 構造化データ＋ i18n ja/en/zh）:
  - `tips/revit-hatch-pattern.html`（→ hatch.html）
  - `tips/pdf-compare-guide.html`（→ pdf_compare.html）
  - `tips/pdf-to-excel-guide.html`（→ pdf-to-excel/web/）
  - `tips/index.html`（一覧ランディング）
- **ナビ導線**: `includes/sidebar.html`・`index.html`（インラインサイドバー＋カテゴリーカード）に「使い方ガイド」追加。
  アイコンは `images/site-icons/icon-practical.png`。翻訳ブロックは `translations.tipsNav`。
- ツールページ最下部（`</main>`直前＝リピーターの邪魔にならない位置）に解説文を追加（`translations.toolAbout`、`.tool-about`）。
- 翻訳ブロック: `tipsHatch` / `tipsPdf` / `tipsExcel` / `tipsNav` / `toolAbout`（全て `Object.assign` 登録・ja/en/zh）。

### js/main.js キャッシュバスター（i18n反映の恒久対策）
- 症状: main.js を `?v=` 無しで読んでいたため、翻訳追加後も CDN/ブラウザが**旧 main.js を配信**し「多言語化されない」ように見えた。
- 対策: **全HTML(29)の `js/main.js` 参照に `?v=20260703-1` を付与**。翻訳更新時はこのバージョンを bump する運用。
- `INCLUDES_VERSION` も `20260703-1` に更新。

### ニュースページ（news.html / ai-news.html）
- **既に `<meta name="robots" content="noindex, nofollow">` 設定済み＋sitemap未登録**（過去セッション対応）。
- ただし **RSSで他サイトの見出し・サムネイルを転用した集約コンテンツ**。noindexでも「価値の低い集約」印象は残りうる。
  → **2026/07/05 対応済み: サイドバー(`includes/sidebar.html`)＋ホーム(`index.html`)のインラインサイドバー＆カテゴリーカードからナビ除去（可逆）**。
  ページ本体・`noindex`・RSS更新ワークフローは残置。翻訳キー(`index-tab-knowledge`等)も復活用に残置。`INCLUDES_VERSION`＝`20260705-1`、全HTMLの `js/main.js?v=20260705-1` に bump 済み。

### Search Console 運用メモ
- サイトマップ送信: 入力欄には **`sitemap.xml` だけを手打ち**（フルURL貼付やhttps://付与で「アドレスが無効」エラーになる）。
  2026/07/05 に **正常処理（検出27ページ）** 確認済み。
- 個別インデックス促進: **URL検査 → インデックス登録をリクエスト**（1日約10件上限）。Tips 4URLをリクエスト済み。
- 既存/表示は `http://28tools.com` プロパティ由来のものが混在。live sitemap は `https://28tools.com/sitemap.xml` で正常配信。

### 残課題 / 次アクション候補
- ~~ニュース集約ページの削除 or ナビ除外~~ → **2026/07/05 ナビ除去で対応済み**。
- インデックス状況を1〜2週間後に再確認 → 「登録済み」増加後に AdSense 再審査。
- **外部シグナル作り（X告知で実アクセス）← 次セッションの主作戦（下記）**／Webツール系のオリジナル記事を継続追加。
- `hatch.html` の既存翻訳漏れ（`hatch-x` / `hatch-y` / `hatch-grout-x` / `hatch-grout-y` が main.js に未定義。正: `hatch-x-spacing` 等）。今回スコープ外で未対応。

### 次セッション予定 — X（Twitter）告知作戦

**目的**: 実アクセス（外部シグナル）を作り、Googleのインデックス評価を押し上げ、AdSense再審査に備える。副次的にツールの認知拡大。

**基本方針**:
- ターゲット＝日本の建築設計・BIM・Revit・施工図ユーザー。
- 発信内容＝①各ツール紹介 ②Tips記事 ③作図の時短小ネタ ④更新情報。価値提供中心（宣伝連投は凍結リスク）。
- ハッシュタグ例: `#Revit` `#BIM` `#建築` `#施工図` `#AutoCAD` `#意匠設計` `#構造設計` `#ゼネコン`。
- 頻度＝1日1〜3投稿。平日昼(12-13時)・夜(18-22時)が有効。
- ⚠️ **この環境にX連携は無い＝Claudeは投稿できない**。次セッションの成果物は「**コピペで即投稿できる文面＋運用手順**」。実投稿はユーザーが行う。

**次セッションの成果物**:
1. アカウント設計（アカウント名候補・プロフィール文ja・固定ツイート案・アイコン方針）
2. 即投稿できる日本語ツイート 20〜30本（各ツール/Tips記事の紹介・作図時短小ネタ。URL・ハッシュタグ込み・140字以内）
3. 初回2週間の投稿カレンダー（曜日×時間帯×ネタ）
4. エンゲージ戦略（絡むべきアカウント像・有益リプの型・ハッシュタグ一覧）

**告知対象URL**:
- サイト: `https://28tools.com/` ／ 使い方ガイド一覧: `https://28tools.com/tips/`
- ハッチング: 記事 `https://28tools.com/tips/revit-hatch-pattern.html` ＋ ツール `https://28tools.com/hatch.html`
- PDF比較: 記事 `https://28tools.com/tips/pdf-compare-guide.html` ＋ ツール `https://28tools.com/pdf_compare.html`
- 構造図Excel化: 記事 `https://28tools.com/tips/pdf-to-excel-guide.html` ＋ ツール `https://28tools.com/pdf-to-excel/web/`
- アドイン: `https://28tools.com/addins.html`
...
## プロジェクト情報
- リポジトリ: 28yu/28tools-download ／ 本番: https://28tools.com
- ガイド: CLAUDE.md を参照（特に「2026/07 セッションで得た知見」＋「次セッション予定 — X告知作戦」）

## 今回の作業: X（Twitter）告知作戦
背景: AdSense審査が「有用性の低いコンテンツ」で却下続き。真因はインデックスの伸び悩み（登録済み2/未登録21）。
時間＋オリジナル記事に加え、外部シグナル（実アクセス）を作るためXで告知する。

やってほしいこと:
1. Xアカウント設計（名前候補・プロフィール文ja・固定ツイート案・アイコン方針）
2. 即投稿できる日本語ツイート20〜30本（各ツール/Tips記事の紹介・作図時短小ネタ。URL・ハッシュタグ込み・140字以内）
3. 初回2週間の投稿カレンダー（曜日×時間帯×ネタ）
4. エンゲージ戦略（絡む相手像・有益リプの型・ハッシュタグ一覧）

制約: この環境にX連携は無くClaudeは投稿できない → コピペで即投稿できる文面＋運用手順を作る（実投稿は自分でやる）。凍結回避のため連投しすぎない・価値提供中心。

告知対象URLは CLAUDE.md「X告知作戦」の一覧を参照。
```
