...
このリポジトリは **public** です。以下はここに書かず、Notion の「28tools 開発ハブ」
(page_id: `370d9148-e297-8186-9c63-e60ae9c79b01`) 配下で管理します。
必要になったら `notion-fetch` で読んでください。

| 内容 | Notion ページ (page_id) |
|---|---|
| AdSense 審査状況・却下履歴、問い合わせ対応フロー、運営メール `info@28tools.com` の DNS 設定 | 運営メモ `3c7d9148-e297-8109-9211-dca5538ae059` |
| X 告知作戦（アカウント設計・ツイート31本・投稿カレンダー・エンゲージ戦略） | X（旧Twitter）告知作戦 `396d9148-e297-8132-bd5a-d507ae714f3b` |
| X 投稿キュー（Typefully/Buffer 流し込み用・返信テンプレ） | X 投稿キュー `3c7d9148-e297-8116-a848-f2805e2871d4` |
| SEO 強化プラン / AdSense 収益ポテンシャル分析 | SEO強化プラン `391d9148-e297-81c6-b5a0-ebfbf992d3ef` |
| Qiita 記事下書き・コミュニティ投稿テンプレ（旧 `promotion/`） | 告知素材アーカイブ `3c7d9148-e297-8134-9caf-db9a68f25901` |

**運用ルール**:
- ⚠️ **実の鍵（PAT / API キー / service key）は Notion にも置かない**。
  GitHub Secrets / Cloudflare Secrets / パスワード管理アプリに置き、Notion には「どこにあるか」だけ書く。
- 個人のメールアドレス・審査の却下履歴・集客戦略は、このファイルに書き戻さない。
- **技術ノウハウ（OCR・CSS・キャッシュ運用など）はこのファイルに残す**方針。
  コードを書いている最中に必要で、機密性も無いため。
- 開発ドキュメントは `_config.yml` でサイト配信から除外済み
  （GitHub 上では public のままなので、機密は上記のとおり Notion 側に置くこと）。
...
- 📋 **審査ステータス・却下履歴・再申請の手順は Notion の「運営メモ」**を参照
  （冒頭「機密情報の参照先（Notion）」の索引から辿れます）
...
## 2026/07 セッションで得た知見 — X 流入計測の実装

集客戦略そのもの（アカウント設計・ツイート文面・投稿カレンダー・エンゲージ戦略）と、
AdSense の審査経緯・問い合わせ対応の記録は **Notion に移設済み**
（冒頭「機密情報の参照先（Notion）」の索引を参照）。ここにはコード側の事実だけ残す。

- **X アカウント**: `@28tools_com`（表示名 `28Tools｜Revit作図サポート`）
- **`analytics/dashboard.html`**: PV タブに「X経由の訪問」KPI（`id=kpi-pv-x`）。
  Supabase の `referrer`（t.co / x.com）と GA4 の `source`（x / twitter / utm_source=x）を合算する。
  判定ロジックは `isXSource()` に集約（`analytics/dashboard.html` 930 行目付近）。
- ⚠️ `js/main.js` の `logPageview()` は `page` に**クエリを含めない**実装 →
  **utm 自体は Supabase に残らない**（X は t.co リファラーで拾えるので Web 流入は KPI に反映される）。
  アプリ流入までリアルタイムで完全計測したい場合は `logPageview` に `location.search` を
  送る小改修が必要（未実施）。
- **反映タイミング**: t.co 経由の Web 流入はリアルタイム（Supabase）、
  GA4 分は日次同期（`ga4-sync` cron）後に加算。
- **透過 PNG の注意**: サイトの favicon をそのまま X アイコンにすると、
  **ダークテーマで背景が黒く抜ける**。白背景を焼き込んだ版を用意すること。

**残タスク**: X のヘッダー画像（1500×500）が未設定。青焼き方眼の意匠を活かす方向で再検討。
...
## 2026/09 セッションで得た知見 — 累計PVの集計バグ（analytics/dashboard.html）

「累計PVが全然増えない」という症状を調査し、集計に **4 つの誤り**を発見して修正した。

### ① Supabase の「Max rows」で取得が 1000 件に打ち切られていた（本命）

`supaFetch()` は `?...&limit=10000` で全件を取るつもりだったが、**Supabase(PostgREST) は
プロジェクト設定の「Max rows」（既定 1000）で返却行数を強制的に打ち切る**。
`limit` に何を書いても 1000 件しか返らない。

そのため `pageviews` が 1000 行を超えた時点で、
**新しい行が入るたび古い行が押し出され、`data.length` が 1000 のまま固定**される。
＝「今週PV / 今月PV は動くのに、累計PVだけ止まっている」という症状になる。

**対策**: `supaFetch()` を **1000 件ずつのページング取得**に変更（`offset = 取得済み件数` で回し、
0 件が返ったら終了）。並び順が同値の行でページ間の取りこぼしが起きないよう、
order に **`id.desc` を第2キー**として必ず付ける。
サーバ側 Max rows が 1000 未満に変更されても正しく動く。

⚠️ 今後 `downloads` / `pageviews` / `ga4_history` を REST で読むコードを足すときは、
**必ず `supaFetch()` を経由**すること（生 fetch + `limit=N` は 1000 で黙って切られる）。

### ② GA4 と自前計測を単純加算していた（期間の二重計上）

- GA4 (`ga4_history`): 2026-01-25 〜 現在
- 自前計測 (`pageviews`): 2026-06-02 〜 現在

両者は**同じアクセスを二重に記録している**（日別に並べるとほぼ同じ数値）。
`累計PV = GA4合計 + Supabase件数` は重複期間を丸ごと二重計上していた。

**対策**: 自前計測の最古の日 `measStartDate`（JST）を境に、
**それより前は GA4 (`ga4Legacy`)、それ以降は自前計測**、と期間で切り分けて合算する。
ページランキング・国別・X経由 KPI・PV推移チャートにも同じ切り分けを適用。
※ 直帰率 / 平均滞在時間 / デバイス / 流入元は **GA4 にしかない指標**なので、
　 これらは従来どおり `allGA4`（全期間）を使う。

### ③ ツール操作イベントを PV として数えていた

`logToolEvent()` は `pageviews` テーブルに `page = '/_event/<key>'` で書き込む。
`renderPV(allPV)` はこれを除外していなかったため、
ハッチDL・PDF比較実行などの操作が「ページビュー」に混入していた（発見時 280 件）。

**対策**: init で `allPVReal = allPV.filter(r => !page.startsWith('/_event/'))` を作り、
PV 集計は `allPVReal`、ツールタブは従来どおり `allPV` を使う。

### ④ PV日別推移だけ UTC 日付で集計していた

`renderPVTrendChart()` の日別集計が `r.ts.slice(0,10)`（＝**UTC 日付**）を使っていた。
他の箇所（時間別・日付ピッカー）は `jstDateStr()` で JST。
**JST 0〜9時のアクセスが前日にずれて**カウントされていた。→ `jstDateStr()` に統一。

### 修正後の想定値（2026-09-06 時点）

| 項目 | 修正前 | 修正後 |
|---|---|---|
| 累計PV | GA4(2,719) + 計測(最大1000で頭打ち) | **2,867** = GA4(〜2026-06-02) 1,096 + 計測 1,771 |
