...
> 📄 **運営・機密情報は非公開リポジトリに分離**: AdSense 審査状況・メール/DNS 設定・各種キー・集客(X)戦略などの運営メモは 非公開リポジトリ `28yu/28tools-private`（`opsnotes.md`）に保管しています。公開リポジトリのこのファイルには**開発ガイドのみ**を残しています。
...
**現状**: 一般公開中。高精度版（Gemini・ユーザー自身の API キー）は動作確認済み。
...
- **即時反映は実装済み**: アドイン開発リポジトリ（28yu/Revit-Add-ins）の
  ...
  認証は Revit-Add-ins 側のリポジトリ Secret（28tools-download への書き込み権限を持つ fine-grained PAT。詳細は非公開リポジトリ参照）。
